...
int ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header)
{
  ...
  if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
    ...
    for (int i = 0; i < nElements; i ++)
      ...
      igtlFloat32 newPosition[3];
      for (int j = 0; j < 3; j++) {
          newPosition[j] = pos[j] + 10;
      }

      igtl::PointElement::Pointer newPoint;
      newPoint = igtl::PointElement::New();
      newPoint->SetName(pointElement->GetName());
      newPoint->SetGroupName(pointElement->GetGroupName());
      newPoint->SetRGBA(rgba);
      newPoint->SetPosition(newPosition);
      newPoint->SetRadius(pointElement->GetRadius());
      newPoint->SetOwner(pointElement->GetOwner());


      pointMsgSend->AddPointElement(newPoint);
    }
    igtl::Sleep(2000);
    pointMsgSend->Pack();
